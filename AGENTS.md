...
This repository contains a Docker-first MCP server for OkoCRM.
...
- API target: OkoCRM REST API v2 (`https://api.okocrm.com/v2`)
- Required env: `OKOCRM_API_TOKEN`

## Architecture Notes

- Tool registration is centralized in `src/server.ts`.
- Each tool lives in its own file under `src/tools/`.
- HTTP calls are implemented in `src/okocrm-request.ts`.
